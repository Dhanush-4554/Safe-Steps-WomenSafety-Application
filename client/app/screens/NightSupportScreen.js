...
export default function NightSupportScreen({ navigation }) {
  ...
  const startLocationUpdates = async () => {
    ...
    if (foregroundStatus !== 'granted') {
      Alert.alert('Permission required', 'Foreground location permission is required to use this feature.');
      setLoading(false);
      return;
    }
  ...
  const enableMic = async () => {
    try {
      ...
      Alert.alert('Microphone Enabled', 'Microphone recording has been enabled.');
  ...
  const stopMic = async () => {
    try {
      ...
      Alert.alert('Microphone Stopped', 'Microphone recording has been stopped.');
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.micControls}>
        <TouchableOpacity
          style={[styles.micButton, { backgroundColor: isMicEnabled ? '#d3d3d3' : '#b0b0b0' }]} // Grey background, lighter when enabled
          onPress={enableMic}
